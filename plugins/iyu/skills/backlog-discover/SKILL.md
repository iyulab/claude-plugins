...
**`state.json` — schema and cadence semantics:
[references/state-schema.md](${CLAUDE_SKILL_DIR}/references/state-schema.md).** It defines the
per-activity cadence clock, the emergent-pool rotation pointer, and the first-run split that keeps a
cold start from touching every lane at once. P0 reads the file, P1 runs the due-check against it, and
P8 writes it back — each of those steps names what it needs, and this reference carries the exact
rule. **Read it before reading or writing `state.json`**; do not reconstruct the schema from memory.
...
schema in [references/state-schema.md](${CLAUDE_SKILL_DIR}/references/state-schema.md) (all
`lastRunUtc: null` / `[]`) — do not write it to disk yet (P8 writes it, and
only if not `--dry-run`).
