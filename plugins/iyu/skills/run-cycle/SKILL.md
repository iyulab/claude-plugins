    ...
    - hooks:
        - type: agent
          ...
          prompt: |
            ...
            4. Respond ALLOW if: completed >= budget; OR a run-fatal `HUMAN-NEEDED:` was emitted; OR every remaining candidate is done or parked in the ledger (no unblocked autonomous work anywhere) AND the latest log carries `FRONTIER-EXHAUSTED:` AND the backlog audit below passes. Before ALLOW on ANY path, Glob `<logs dir>/RUN-SUMMARY-*.md` — inside `<logs dir>` itself, beside the `cycle-*.md` files, NOT in its parent — and confirm one covers this run: a `Run: cycles {a}–{b} · ended {date}` header line whose range reaches this run's `Start:`. If no file there carries a `Run:` line at all (a report written before that header existed), any `RUN-SUMMARY-*.md` in `<logs dir>` satisfies the check — fail toward allowing, as step 1 does. The filename date is never the test (a run that crosses midnight is dated by the day it ends). If none satisfies, respond BLOCK with the report instruction below.
            4a. Backlog audit — ONLY on the `FRONTIER-EXHAUSTED:` allow path (skip it when completed >= budget or a run-fatal blocker ended the run). That token is the run's own **claim** that nothing unblocked remains; audit the claim against a file the run is not writing right now — open `<logs dir>/../ROADMAP.md`, and if that does not exist try `<logs dir>/../../ROADMAP.md` (an umbrella keeps one central roadmap above its per-submodule log directories). Remaining phase work only falsifies the claim when it is work this run could have done: BLOCK only if a phase is plainly **neither completed, nor parked in the log's Blocked-on-Human ledger, nor recorded under Pending Human Decisions as discussion-class** — those last two are human-gated by design, and blocking on them names work the run cannot act on. When you do block, **name the phases in the reason**. Judge only what is plain — you are spotting obviously-remaining autonomous work, not adjudicating wording. **No roadmap found, empty, or genuinely ambiguous → the audit PASSES**: fail toward allowing, as step 1 does, since a user escapes a premature allow by invoking again but cannot escape a run that will not end.
